...
export default function Employees() {
    ...
    return (
        <div>
            <h1 className="text-xl md:text-2xl font-bold mb-1" style={{ color: "#fff" }}>Employees</h1>
            <p className="text-sm mb-6" style={{ color: "#9ca3af" }}>Manage your team</p>

            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 mb-4">
                <input value={search} onChange={(e) => setSearch(e.target.value)}
                    placeholder="Search by name, phone or email..."
                    className="px-4 py-2 rounded-lg text-sm w-full sm:w-72"
                            ...
                            {filteredEmployees.length === 0 ? (
                                ...
                                filteredEmployees.map((emp) => (
                                    <tr key={emp._id} style={{ borderBottom: "1px solid #1e2433" }}>
                                        <td className="px-4 md:px-5 py-3 text-sm font-medium" style={{ color: "#fff" }}>{emp.name}</td>
                                        <td className="px-4 md:px-5 py-3 text-sm" style={{ color: "#9ca3af" }}>{emp.contact}</td>
                                        <td className="px-4 md:px-5 py-3 text-sm" style={{ color: "#9ca3af" }}>{emp.email}</td>
                                        {role === "admin" && (
            ...
            {role === "admin" && showModal && createPortal(
                <div className="fixed inset-0 flex items-center justify-center z-50 px-4" style={{ background: "rgba(0,0,0,0.7)" }}>
                    <div className="rounded-xl p-6 w-full max-w-sm shadow-xl" style={{ background: "#151921", border: "1px solid #1e2433" }}>
