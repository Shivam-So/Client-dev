...
export default function AdminDashboard() {
    ...
    return (
        <div>
            <h1 className="text-2xl font-bold mb-1" style={textWhite}>Dashboard</h1>
            <p className="text-sm mb-6" style={textMuted}>Welcome to your admin panel</p>


            <div className="grid grid-cols-2 gap-4 mb-6" style={{ maxWidth: 600 }}>
                <div className="p-5" style={cardStyle}>
                    <div className="flex items-center gap-3 mb-3">
                        <div className="w-10 h-10 rounded-lg flex items-center justify-center" style={{ background: "#1e3a5f" }}>
                            <Users size={20} color="#60a5fa" />
                        </div>
                        <p className="text-sm" style={textMuted}>Total Users</p>
                    </div>
                    <p className="text-3xl font-bold" style={textWhite}>{totalUsers}</p>
                ...
                <div className="p-5" style={cardStyle}>
                    <div className="flex items-center gap-3 mb-3">
                        <div className="w-10 h-10 rounded-lg flex items-center justify-center" style={{ background: "#1a2e1a" }}>
                            <FileText size={20} color="#4ade80" />
                        </div>
                        <p className="text-sm" style={textMuted}>Total Forms</p>
                    </div>
                    <p className="text-3xl font-bold" style={textWhite}>{totalForms}</p>
                    <div className="mt-3 h-1 rounded-full" style={{ background: "#1e2433" }}>
            ...
            <div style={cardStyle}>
                <div className="flex items-center justify-between px-5 py-4 border-b" style={{ borderColor: "#1e2433" }}>
                    <h2 className="font-semibold" style={textWhite}>Recent Users</h2>
                        ...
                        {users.length === 0 ? (
                            ...
                            users.map((user, i) => (
                                <tr key={i} style={{ borderBottom: "1px solid #1e2433" }}>
                                    <td className="px-5 py-3 text-sm font-medium" style={textWhite}>{user.name}</td>
                                    <td className="px-5 py-3 text-sm" style={textMuted}>{user.email}</td>
                                    <td className="px-5 py-3 text-sm" style={textMuted}>{user.contact || "N/A"}</td>
                                    <td className="px-5 py-3 text-sm" style={textMuted}>{new Date(user.createdAt).toLocaleDateString()}</td>
                                </tr>
                            ))
                        )}
                    </tbody>
