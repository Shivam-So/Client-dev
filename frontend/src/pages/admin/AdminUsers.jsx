...
export default function AdminUsers() {
    ...
    return (
        <div>
            <h1 className="text-xl md:text-2xl font-bold mb-1" style={{ color: "#fff" }}>Users</h1>
                            ...
                            {currentUsers.length === 0 ? (
                                ...
                                currentUsers.map((user) => (
                                    <tr key={user._id} style={{ borderBottom: "1px solid #1e2433" }}>
                                        <td className="px-4 md:px-5 py-3 text-sm font-medium" style={{ color: "#fff" }}>{user.name}</td>
                                        <td className="px-4 md:px-5 py-3 text-sm" style={{ color: "#9ca3af" }}>{user.email}</td>
                                        <td className="px-4 md:px-5 py-3 text-sm" style={{ color: "#9ca3af" }}>{user.contact || "N/A"}</td>
                                        <td className="px-4 md:px-5 py-3 text-sm" style={{ color: "#9ca3af" }}>{new Date(user.createdAt).toLocaleDateString()}</td>
                                    </tr>
                                ))
                            )}
                        </tbody>
